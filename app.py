...
def segments_twenty_minutes(mp3_file_path, video_id):
    ...
    while start < n:
        audio_clip = AudioFileClip(mp3_file_path)
            ...
            start + segment_duration_seconds, n
        )  # Ensure the segment doesn't exceed the video duration
...
def get_transcript(file_path, language_code, video_id):
    ...
    if (
        mp3file_size_bytes < whisper_default_size
    ):  # check if given file is smaller than 25 MB
        result = model.transcribe(file_path, language=language_code)["text"]
        os.remove(file_path)
        return result
    else:
        transcribed_text = []
        segmented_files_dir = segments_twenty_minutes(
            file_path, video_id
        )  # function call for segmennting video into small segments
        ...
        for index in range(len(files_list)):
            ...
            result = model.transcribe(audio_segment_path, language=language_code)[
                "text"
            ]
